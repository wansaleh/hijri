...
*/
function toGregorian(hy: number, hm: number, hd: number) {
...
*/
function isValidSolarHijriDate(hy: number, hm: number, hd: number) {
  return (
    hy >= -61 &&
    hy <= 3177 &&
    hm >= 1 &&
    hm <= 12 &&
    hd >= 1 &&
    hd <= solarHijriMonthLength(hy, hm)
  );
}
...
*/
function isLeapSolarHijriYear(hy: number) {
...
*/
function solarHijriMonthLength(hy: number, hm: number) {
  if (hm <= 5) return 30;
  if (hm >= 7) return 31;
  // here where hm = 6.
...
*/
function solarHijriCal(hy: number) {
  const gy = hy + 621,
    pgy = gy + 1;

  return {
    leap: (pgy % 4 == 0 && pgy % 100 != 0) || pgy % 400 == 0,
    gy: gy,
    startDate: 23, // 23 Sep
  };
}
...
function h2j(hy, hm, hd) {
  const r = solarHijriCal(hy),
    leap = r.leap ? 0 : -1;
  return (
    g2j(r.gy, 9, r.startDate) +
    (hm - 1) * 30 +
    div(hm, 7) * (hm - 7) +
    hd -
    1 +
...
*/
function j2h(hdn: number): { hy: number; hm: number; hd: number } {
  const gy = j2g(hdn).gy; // Calculate Gregorian year (gy).
  ...
  const hdn1l = g2j(gy, 9, r.startDate); // Julian day number of 1st Libra(Mizān)
...
*/
function j2g(hdn: number): { gy: number; gm: number; gd: number } {
  let j = 4 * hdn + 139361631;
...
export {
  ...
  solarHijriCal,
  // h2j, // solarHijri to Julian
  // j2h, // Julian to solarHijri
  // g2j, // Gregoria to Julian
  // j2g, // Julian to Gregoria
};
